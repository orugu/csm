...
| `csm --add <ID@IP> [-p 포트]` | 한 줄로 바로 Host 추가 (그룹 없이/기타로, 포트 생략 시 SSH 기본값 22) |
...
```sh
csm --add root@192.168.0.10          # Port 생략 -> SSH 기본값 22
csm --add root@192.168.0.10 -p 2222  # 포트 지정
csm --add -p 2222 root@192.168.0.10  # -p는 앞/뒤 순서 상관없음
```

`ID@IP` 형식으로 파싱해서 `User`/`HostName`을 채우고, 별칭은 IP를 그대로 쓴다. `-p`를
생략하면 `~/.ssh/config`에 `Port` 줄 자체를 안 남긴다(SSH 기본값 22가 그대로 적용됨).
그룹 없이(`csm --mkdir`의 "기타" 취급과 동일하게) 추가된다.
