...
- **`csm --add` 추가 (v1.11)**: `csm --add <ID@IP> [-p <포트>]` — 대화형 `--mkdir` 없이
  한 줄로 바로 Host를 추가하는 명령. 구현/검증 메모:
  - `-p`는 `<ID@IP>` 앞이든 뒤든 파싱되게 일반 옵션 루프로 처리했고, **`-p`만 주고 값을
    안 주면(마지막 인자인 경우) 즉시 에러내고 중단**하도록 방어했다 — 이건 net-dashboard의
    `-p` 값 검증 누락이 실제 서비스 장애(기존 인스턴스 강제종료)로 이어졌던 사고를 겪은
    직후라, 값이 필요한 옵션은 항상 이 패턴으로 만든다.
  - `-p`를 생략하면 `~/.ssh/config`에 `Port` 줄 자체를 안 쓴다(SSH 기본값 22가 자연히
    적용됨) — "포트 지정 안 하면 22번 포트"라는 요구사항을 "결과적으로 22로 접속됨"으로
    해석했다("Port 22"를 파일에 명시하라는 뜻이 아니라고 판단).
  - **새 Host 블록은 파일의 맨 앞(첫 `# csm-group:` 주석보다 앞)에 삽입한다** — `csm`/
    `csm --graph`의 그룹 파싱이 `BEGIN{group="기타"}`로 시작해서 첫 csm-group 주석 전까지는
    전부 "기타"로 잡는 구조라, 파일 끝에 붙이면 마지막 그룹 섹션에 잘못 속하게 된다.
  - `python3 -`에는 데이터를 argv로만 넘겼다(`~/.ssh/config`와 `$block` 둘 다 인자로,
    파이프 안 씀) — 이 프로젝트에서 두 번 재발했던 stdin 충돌 함정을 세 번째로 만들지
    않기 위함.
  - 실측 검증(2026-07-31): `@` 없음/앞뒤 어느 한쪽 빈 값/`-p` 값 없음/`-p` 값이 숫자 아님/
    알 수 없는 옵션 6가지 에러 케이스 전부 확인. 정상 케이스는 임시 가짜 `$HOME`으로
    먼저 검증(`-p` 있음/없음/위치 다름/중복 별칭 4케이스, `ssh -G`로 HostName/User/Port
    정확히 잡히는 것과 그룹이 정확히 "기타"로 분류되는 것까지 확인) 후, 실제
    `~/.ssh/config`에 대해서도 테스트 항목 하나(`tester@203.0.113.99 -p 2200`) 추가 →
    기존 15개 호스트 전부 `ssh -G`로 무손실 확인 → 테스트 항목 제거 → 다시 15개로
    원상복구 확인까지 마쳤다.
