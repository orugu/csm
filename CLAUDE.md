...
- **`csm --add` 추가 (v1.11)**: `csm --add <ID@IP> [-p <포트>]` — 대화형 `--mkdir` 없이
  ...
  - 실측 검증(2026-07-31): `@` 없음/앞뒤 어느 한쪽 빈 값/`-p` 값 없음/`-p` 값이 숫자 아님/
    ...
    원상복구 확인까지 마쳤다.
- **v1.11 작업에서 실제로 버그를 하나 남겼다: 상단 주석은 "version 1.11"로 바꿨는데
  `_CSM_VERSION` 변수 자체는 "1.10"으로 안 바뀐 채였다.** v1.12 작업(아래 `-n` 추가) 도중
  코드를 다시 훑다가 발견해서 같이 고쳤다. 이 상태로 있었으면 `csm --update`/`U` 키 알림이
  실제로는 이미 최신인데도 "업데이트 있음"으로 계속 오탐했을 것이다 — **버전을 올릴 때는
  상단 주석 코멘트와 `_CSM_VERSION` 변수 둘 다 반드시 같이 바꿀 것, 하나라도 빠지면
  버전 비교 로직 전체가 조용히 틀어진다.**
- **`csm --add`에 `-n <이름>` 옵션 추가 (v1.12)**: 별칭을 IP 대신 원하는 이름으로 지정.
  `-n`을 생략하면 기존처럼 IP가 별칭이 된다(하위호환). `-p`와 동일한 패턴으로 값 없이
  `-n`만 오면 즉시 에러(`csm --add root@1.2.3.4 -n` 재현 확인). `-n`/`-p` 둘 다 순서
  섞어서 줘도(`csm --add -n x -p 2222 user@ip`) 정상 파싱되는 것 확인. 별칭 중복 체크는
  최종적으로 쓰일 값(`-n` 있으면 그 이름, 없으면 IP) 기준으로 한다. 실측 검증(2026-07-31):
  가짜 `$HOME`에서 5가지 케이스(-n 없음/있음/순서 섞기/값 없음/중복) 확인 후, 실제
  `~/.ssh/config`에도 `-n test-n-option` 케이스로 추가 → `ssh -G`로 확인 → 기존 15개
  호스트 무손실 재확인 → 제거해서 원상복구까지 완료.
